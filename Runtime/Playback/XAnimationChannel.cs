...
namespace XAnimationEngine
{
    ...
    public abstract class XAnimationStatePlaybackInstance
    {
            ...
            XAnimationPlaybackRuntimeOptions options)
        {
            ...
            Speed = ResolvePlaybackSpeed(stateConfig);
            IsLooping = stateConfig?.loop ?? true;
        ...
        public abstract XAnimationChannelState BuildState(float channelWeight, float globalSpeed);
            ...
            bool isConnected)
        {
            ...
            node.stateSpeed = Speed;
            node.globalSpeed = globalSpeed;
            node.speed = Speed * globalSpeed;
    ...
    public sealed class XAnimationSingleStatePlaybackInstance : XAnimationStatePlaybackInstance
    {
        ...
        internal XAnimationSingleStatePlaybackInstance(
            int playbackId,
            string channelName,
            XAnimationCompiledSingleState state,
            Animator animator,
            XAnimationCompiledClip clip,
            AnimationClipPlayable playable,
            bool isTemporaryState,
            XAnimationPlaybackRuntimeOptions options)
            : base(playbackId, channelName, state?.Key, XAnimationStateType.Single, isTemporaryState, state?.Config, options)
        {
        ...
        protected override void PrepareStateFrame(float deltaTime, float playableSpeedScale, XAnimationContext context)
        {
            m_PreviousTotalNormalizedTime = m_TotalNormalizedTime;
            SetPlayableSpeed(Speed * playableSpeedScale);
            m_TotalNormalizedTime += deltaTime * Speed / m_ClipLength;
        }
        ...
        public override XAnimationChannelState BuildState(float channelWeight, float globalSpeed)
        {
            ...
            bool isConnected)
        {
            ...
            PopulateDebugPlaybackNode(node, inputIndex, inputWeight, channelWeight, globalSpeed, isConnected);
    ...
    public sealed class XAnimationBlend1DStatePlaybackInstance : XAnimationStatePlaybackInstance
    {
        ...
        internal XAnimationBlend1DStatePlaybackInstance(
            ...
            bool isTemporaryState,
            XAnimationPlaybackRuntimeOptions options)
            : base(playbackId, channelName, state?.Key, XAnimationStateType.Blend1D, isTemporaryState, state?.Config, options)
        {
            m_Animator = animator ? animator : throw new ArgumentNullException(nameof(animator));
            m_Graph = graph;
            m_State = state ?? throw new ArgumentNullException(nameof(state));
            m_Clips = clips ?? throw new ArgumentNullException(nameof(clips));
            m_Mixer = AnimationMixerPlayable.Create(graph, m_Clips.Length);
        ...
        protected override void PrepareStateFrame(float deltaTime, float playableSpeedScale, XAnimationContext context)
        {
            ...
            m_TotalNormalizedTime += deltaTime * Speed / blendedClipLength;
    ...
    public sealed class XAnimationBlend2DSimpleDirectionalStatePlaybackInstance : XAnimationStatePlaybackInstance
    {
        ...
        internal XAnimationBlend2DSimpleDirectionalStatePlaybackInstance(
            ...
            bool isTemporaryState,
            XAnimationPlaybackRuntimeOptions options)
            : base(playbackId, channelName, state?.Key, XAnimationStateType.Blend2DSimpleDirectional, isTemporaryState, state?.Config, options)
        {
    ...
    public sealed class XAnimationBlend2DFreeformDirectionalStatePlaybackInstance : XAnimationStatePlaybackInstance
    {
        ...
        internal XAnimationBlend2DFreeformDirectionalStatePlaybackInstance(
            ...
            bool isTemporaryState,
            XAnimationPlaybackRuntimeOptions options)
            : base(playbackId, channelName, state?.Key, XAnimationStateType.Blend2DFreeformDirectional, isTemporaryState, state?.Config, options)
        {
            _ = animator ? animator : throw new ArgumentNullException(nameof(animator));
            m_Graph = graph;
            m_State = state ?? throw new ArgumentNullException(nameof(state));
    ...
    public sealed class XAnimationChannel
    {
            ...
            float globalSpeed)
        {
            ...
            node.details = $"Layer: {LayerType} | Channel Weight: {m_ChannelWeight:0.###}";
